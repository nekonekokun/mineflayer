...
function inject(bot) {
  ...
  function nextFrame(deltaSeconds) {
    ...
    var posBlock=bot.blockAt(pos.offset(0, 0,0));
    ...
    if(movementForward || movementRight) {
      ...
      if(controlState.sneak){
        acceleration.x *= physics.sneakSpeed;
        acceleration.z *= physics.sneakSpeed;
      }
      else if(controlState.sprint) {
    ...
    if(0){
    //if(posBlock && posBlock.type===65){
      airFriction = 0.1;
      terminalVelocity = 2;
      bot.entity.onGround = true;
      bot.entity.timeSinceOnGround = WAIT_TIME_BEFORE_NEW_JUMP + 1;
    }
    acceleration.y -= physics.gravity * airFriction;
    ...
    if(vel.x !== 0) {
      ...
      //if(0) {
      if(block = collisionInRange(boundingBoxMin, boundingBoxMax)) {
        if(0){
        //if(posBlock && posBlock.type===65){
          vel.y = 5;
        }
        //console.log("x collision to " + block.displayName);
        //console.log("(pos.y - blockY) = " + (pos.y - blockY));
        if((block.boundingBoxMax.y - (pos.y - blockY)) <= 0.51)
        //if(false)
        {
          //console.log("x collision to " + "half slab");
          var block_height = block.boundingBoxMax.y;
          pos.y = blockY + block_height * 1.001;
    ...
    if(vel.z !== 0) {
      ...
      //if(0) {
      if(block = collisionInRange(boundingBoxMin, boundingBoxMax)) {
        //console.log("z collision to " + block.displayName);
        //console.log(posBlock);
        if(0){
        //if(posBlock && posBlock.type===65){
          //console.log("climbing " + posBlock.displayName);
          vel.y = 5.0;
          //pos.z = blockZ + (vel.z < 0 ? 1 + physics.playerApothem : -physics.playerApothem) * 5;
          bot.entity.onGround = true;
          bot.entity.timeSinceOnGround = WAIT_TIME_BEFORE_NEW_JUMP + 1;
        }
        ...
        //if(false)
        {
          //console.log("z collision to " + "half slab");
          var block_height = block.boundingBoxMax.y;
          //console.log("block_height = " + block_height);
          //console.log("blockY = " + blockY);
          pos.y = blockY + block_height * 1.1;
          //console.log(" pos.y = " +   pos.y);

          //pos.z += vel.z * deltaSeconds;
          vel.y = 0.0;
          climbing_stairs = true;
          //pos.z = blockZ + (vel.z < 0 ? 1 + physics.playerApothem : -physics.playerApothem) * 0.99;
        }
        else{
    ...
    vel.y = math.clamp(-terminalVelocity, vel.y, terminalVelocity);
    //vel.y  = 0.05;
    //console.log("vel.y = " + vel.y);
    if(vel.y !== 0) {
      pos.y += vel.y * deltaSeconds;
      var playerHalfHeight = physics.playerHeight / 2;
      var blockYReal = pos.y + playerHalfHeight + math.sign(vel.y) * playerHalfHeight;
      var blockY = Math.floor(pos.y + playerHalfHeight + math.sign(vel.y) * playerHalfHeight);
      boundingBoxMin = new Vec3(boundingBox.min.x, blockYReal, boundingBox.min.z);
      boundingBoxMax = new Vec3(boundingBox.max.x, blockYReal, boundingBox.max.z);
      //console.log("blockY" + pos.y + playerHalfHeight + math.sign(vel.y) * playerHalfHeight);
      //console.log("y boundingbox = " + boundingBoxMin);
      var block = null;
      //if(0) {
      if(block = collisionInRange(boundingBoxMin, boundingBoxMax)) {
        //console.log("y collision to " + block.displayName);
        var block_height = block.boundingBoxMax.y;
        //if(!climbing_stairs)
        pos.y = blockY + (vel.y < 0 ? block_height : -physics.playerHeight) * 1.001;
        ...
        for(cursor.z = boundingBoxMin.z; cursor.z <= boundingBoxMax.z; cursor.z++) {
          ...
          if(block){
            if(block.boundingBox === 'block') {
              //console.log("collision to block");
              return block;
            }
            else if(block.boundingBox === 'slab_bottom'){
              //console.log("collision to slab_bottom");
              //console.log(block);
              //console.log("dy = " + (cursor.y - block.position.y));
              //console.log("cursor.y = " + cursor.y);
              //console.log("block.position.y = " + block.position.y);
              if(cursor.y - block.position.y < 0.5) return block;
            }
          }
        }
      }
