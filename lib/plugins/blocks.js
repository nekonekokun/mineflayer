...
function inject(bot,{version}) {
  ...
  function blockAt(absolutePoint) {
    ...
    //augment block bounding box
    var ids_slab = [26, 44, 126, 182, 205];
    var ids_nocollision = [65, 171];
    //console.log("block.type = " + block.type);
    block.boundingBoxMin = new Vec3(0, 0, 0);
    block.boundingBoxMax = new Vec3(1, 1, 1);
    if(ids_slab.indexOf(block.type) >= 0
      && block.half == "bottom"){//}// && block.metadata < 8){
      block.boundingBox = "slab_bottom";
      block.boundingBoxMax = new Vec3(1, 0.5, 1);
    }

    if(ids_nocollision.indexOf(block.type) >= 0)
    {
